...
export class AuthController {
  ...
  async getProfile(@Request() req: RequestWithUser) {
    ...
    if (!userEmail && userId) {
      console.log('[Auth] Email missing in token, fetching from DB for user:', userId);
      const dbUser = await this.prisma.user.findUnique({ where: { id: userId } });
      if (dbUser) userEmail = dbUser.email;
    }

    if (!userEmail) {
      throw new NotFoundException('No se encontró email en el token ni en la base de datos');
    }
