...
export class MailingService {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.adminEmail = this.configService.get<string>('SUPER_ADMIN_EMAILS')?.split(',')[0] || 'andrewlicona1@gmail.com';

        if (apiKey) {
            this.resend = new Resend(apiKey);
        } else {
            this.logger.warn('RESEND_API_KEY not found. Emails will be logged to console instead of sent.');
        }
    }
